...
class LexicalRetriever:
    ...
    def index(self, chunk_ids: List[str], documents: List[str]) -> None:
        ...
        print(f"  [LexicalRetriever] 倒排索引已构建: {len(self._postings)} 个 term → "
              f"{sum(len(p) for p in self._postings.values())} 个 posting")
    ...
    def search(self, query: str, top_k: int = 10,
        ...
        for term in query_tokens:
            if term in self._postings:
                docs = set(self._postings[term].keys())
                if allowed_indices is not None:
                    docs &= allowed_indices
                if docs:
        ...
        for doc_idx in candidates:
            ...
            for term in query_tokens:
                if term not in self._idf:
                    continue
                tf = self._postings.get(term, {}).get(doc_idx, 0)
                if tf == 0:
                    continue
    ...
    def load_index(self, path: str) -> bool:
        ...
        # 倒排索引：优先从缓存加载，缺失则从 _tokenized 重建
        if "_postings" in data:
            self._postings = data["_postings"]
        else:
            self._postings = {}
            for idx, tokens in enumerate(self._tokenized):
                for term in set(tokens):
