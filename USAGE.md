...
检索服务首次启动时需要 JSONL 数据文件。数据文件放置位置：
...
> 首次构建完成后，JSONL 文件不再是启动的必需依赖。索引和全部数据已持久化到 `.cache/` 目录（SQLite + 向量 + BM25 等），后续启动直接从缓存加载，无需 JSONL 文件存在。只有需要更新数据时才需要放回 JSONL 文件。
...
检索服务通过数据指纹判断缓存是否有效，采用双路径策略：

- **JSONL 文件存在时**：对数据目录下所有 JSONL 文件计算 `文件名 + 文件大小 + 修改时间` 的哈希。新增、修改或删除任意 JSONL 文件，指纹就会变化，下次启动时自动触发全量重建索引。
- **JSONL 文件不存在时**：从 `.cache/` 目录中查找最新的有效 manifest，直接恢复缓存。此时不依赖 JSONL 文件，数据从已持久化的 SQLite 数据库加载。

因此加入新 chunk 后：

- 将 JSONL 文件放入数据目录，**重启检索服务**即可，指纹变化会自动触发重建。
- 如果 JSONL 文件已在目录中但想强制重建，删除 `regulatory_docs/.cache/` 目录后重启。
- 如果已删除 JSONL 文件（仅靠缓存运行），放回修改后的 JSONL 文件再重启即可触发重建。
...
**Q: 删除了 JSONL 文件后还能启动吗？**
A: 可以。首次构建完成后，全部数据已持久化到 `.cache/` 目录的 SQLite 数据库中。JSONL 文件不存在时，系统会从缓存 manifest 恢复，直接加载已有索引。只有需要更新数据时才需要放回 JSONL 文件。
