...
class RetrievalAPI:
    ...
    def load(self, source: Union[str, Path],
             ...
             db_path: Optional[str] = None,
             lightweight: bool = False) -> "RetrievalAPI":
        source = str(source)
        ...
        print("=" * 60)
        mode_label = " [轻量模式: 跳过 Dense 向量]" if lightweight else ""
        print(f"  Retrieval API v1 — 加载检索器 ...{mode_label}")
        print("=" * 60)
        ...
        if manifest_path.exists():
            ...
            if self._load_from_cache(cache_dir, data_hash, populate_db, lightweight):
                self._loaded = True
                self.warmup()  # 预热 Dense 模型，避免首次查询 30s 冷启动
        ...
        # ── 3. Dense + 缓存（lightweight 模式跳过）──
        if lightweight:
            print("\n[3/8] 跳过 Dense (向量) 索引（轻量模式）")
            self.dense = None
        else:
            print("\n[3/8] 构建 Dense (向量) 索引 ...")
        ...
        self._loaded = True
        self.warmup()  # 预热 Dense 模型（全量路径已加载过，此处为 no-op）
        doc_count = len(set(m.doc_id for m in self._store._meta_map.values()))
    ...
    def warmup(self) -> None:
        """预热 Dense 嵌入模型（把首次查询的 30s 模型加载提前到启动阶段）

        缓存启动时只加载了向量矩阵，模型本体是懒加载的；
        这里在启动阶段触发 _load_model()，让第一次查询秒回。
        全量构建路径已通过 dense.index() 加载过模型，此处为 no-op。
        """
    ...
    def _save_all_indexes(self, cache_dir: Path, data_hash: str, text_hash: str) -> None:
        ...
        if self.dense is not None:
            files["dense_meta"] = f"dense_meta_{text_hash}.pkl"
            files["dense_emb"] = f"embeddings_{emb_model_tag}_{text_hash}.npy"
        manifest = {
    ...
    def _load_from_cache(self, cache_dir: Path, data_hash: str,
                         populate_db: bool, lightweight: bool = False) -> bool:
        """从缓存加载全部索引，成功返回 True。lightweight=True 跳过 Dense 向量。"""
        manifest_path = cache_dir / f"manifest_{data_hash}.json"
        ...
        try:
            ...
            # 3. Dense（向量 + chunk_ids）—— light 模式跳过
            if lightweight:
                print("  [3/6] 跳过 Dense 向量（轻量模式）")
                self.dense = None
            else:
    ...
    def search_request(self, req: RetrievalRequest) -> List[RetrievalHit]:
        ...
        # ── Phase 2: chunk 类型互补 — 确保 table + cell_fact 兼有 ──
        if req.rerank != RerankMode.CROSS_ENC:  # 不在精排路径上做（精排有自己的逻辑）
            _t_div = _time.time()
            hits = self._ensure_diversity(req, hits)
            print(f"  [Phase 2] 类型互补后 {len(hits)} 条命中  ({_time.time() - _t_div:.3f}s)")
    ...
    def _search_structured(self, req: RetrievalRequest) -> List[RetrievalHit]:
        ...
        if self.neighborhood and req.strategy == RetrievalStrategy.TABLE:
            ...
            sibling_cache: Dict[str, list] = {}  # parent_chunk_id → siblings（去重）
            pending_sibs: Dict[str, tuple] = {}

            for hit in list(hits):
                cid = hit.chunk_id
                if not cid or "cell_" not in cid or "_cell_B" in cid:
                    continue
                # 获取 parent_chunk_id 做缓存键，空值时用 doc_id 兜底
                meta = self._store.get_meta(cid)
                parent_id = meta.metadata.get("parent_chunk_id", "") if meta else ""
                cache_key = parent_id or (f"__doc_{meta.doc_id}" if meta else cid)
                if cache_key not in sibling_cache:
                    sibling_cache[cache_key] = self.neighborhood.get_siblings(cid) or []
                for sib in sibling_cache[cache_key]:
                    sid = sib.get("chunk_id", "")
                    if sid and sid not in hit_ids and sid not in pending_sibs and "_cell_B" in sid:
                        pending_sibs[sid] = (hit.score, list(hit.matched_by), hit.trace)

            if pending_sibs:
                self._store.get_content_batch(list(pending_sibs.keys()))
            added = 0
            for sid, (score, matched_by, trace) in pending_sibs.items():
                sib_result = self._build_result_from_store(sid, score)
                sib_hit = RetrievalHit.from_chunk_result(
                    sib_result,
                    rank=len(hits) + 1,
                    matched_by=matched_by if matched_by else ["table"],
                    trace=trace,
                )
                hits.append(sib_hit)
                hit_ids.add(sid)
    ...
    def _ensure_diversity(self, req: RetrievalRequest,
                          hits: List[RetrievalHit]) -> List[RetrievalHit]:
        """
        chunk 类型互补 — 确保结果中同时有 table（完整性）和 cell_fact（精准度）。

        逻辑：
          1. 如果已有 table/table_section + cell_fact/table_row → 不做处理
          2. 如果只有 table 没有 cell_fact → 从邻域图补入子 cell_fact
          3. 如果只有 cell_fact 没有 table → 从邻域图补入父 table
        """
        if not self.neighborhood or not hits:
            return hits

        existing_ids = {h.chunk_id for h in hits}
        has_table = any(h.chunk_type in ("table", "table_section") for h in hits)
        has_cell = any(h.chunk_type in ("cell_fact", "table_row") for h in hits)
        supplements: List[RetrievalHit] = []

        # 只要有 table/table_section，递归拉 cell_fact 子节点
        if has_table:
            def _collect_cells(chunk_id: str, depth: int = 0) -> List[dict]:
                """递归收集 cell_fact/table_row 子节点"""
                if depth > 2:
                    return []
                result = []
                for child in (self.neighborhood.get_children(chunk_id) or []):
                    ct = child.get("chunk_type", "")
                    if ct in ("cell_fact", "table_row"):
                        result.append(child)
                    elif ct in ("table_section", "table"):
                        result.extend(_collect_cells(child.get("chunk_id", ""), depth + 1))
                return result

            for h in hits:
                if h.chunk_type not in ("table", "table_section"):
                    continue
                for child in _collect_cells(h.chunk_id):
                    cid = child.get("chunk_id", "")
                    if not cid or cid in existing_ids:
                        continue
                    c_result = self._build_result_from_store(cid, h.score)
                    c_hit = RetrievalHit.from_chunk_result(
                        c_result,
                        rank=len(hits) + len(supplements) + 1,
                        matched_by=list(h.matched_by) if h.matched_by else ["neighborhood"],
                        trace=h.trace,
                    )
                    supplements.append(c_hit)
                    existing_ids.add(cid)
                    if len(supplements) >= req.top_k:
                        break
                if len(supplements) >= req.top_k:
                    break

        if not has_table and has_cell:
            # 有 cell 缺整表 → 从邻域图拉父 table
            for h in hits:
                if h.chunk_type not in ("cell_fact", "table_row"):
                    continue
                parent = self.neighborhood.get_parent(h.chunk_id)
                if not parent:
                    continue
                pid = parent.get("chunk_id", "")
                if not pid or pid in existing_ids:
                    continue
                # 优先取 table_section，没有则取 table
                p_result = self._build_result_from_store(pid, h.score)
                p_hit = RetrievalHit.from_chunk_result(
                    p_result,
                    rank=0,  # table 置顶
                    matched_by=list(h.matched_by) if h.matched_by else ["neighborhood"],
                    trace=h.trace,
        ...
        if supplements:
            hits.extend(supplements)
            for i, hh in enumerate(hits, 1):
                hh.rank = i
            print(f"    [类型互补] 补入 {len(supplements)} 条 cell_fact")
    ...
    def _lexical_raw_to_results(self, raw: List[tuple]) -> List[Dict[str, Any]]:
        ...
        """
        # 批量预取 content（1 次 SQL 替代 N 次）
        chunk_id_map: Dict[int, str] = {}
        for doc_idx, _ in raw:
            chunk_id_map[doc_idx] = self._store.get_chunk_id(doc_idx)
        valid_ids = [cid for cid in chunk_id_map.values() if cid]
        contents = self._store.get_content_batch(valid_ids)

        results = []
        for doc_idx, score in raw:
            chunk_id = chunk_id_map.get(doc_idx, "")
            if not chunk_id:
                continue
            meta = self._store.get_meta(chunk_id)
            results.append({
