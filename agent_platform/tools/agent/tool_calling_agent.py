...
## 表格分区识别规则（极其重要）
Excel 表格通常包含多个分区（如"1. 银行业金融机构"、"其中：商业银行合计"、"2. 大型商业银行"等），
每个分区都有相同的指标（如"总资产"、"总负债"），但数值不同。
1. 当问题提到"银行业"总资产/总负债时，必须选择"1. 银行业金融机构"分区的数据
2. 检索结果中每条证据的 metadata.table_name 字段标明了其所属表格分区
3. 同一指标在不同分区有不同数值时，必须选择与问题最匹配的分区
4. 绝对不能混用不同分区的数据（如用"其中：商业银行合计"的Q1值减去"1. 银行业金融机构"的Q4值）
5. 涉及两处取数计算时，两个数值必须来自同一分区
6. 使用 get_cell_value 工具时可指定 table_name 参数精确过滤

## 数值计算方向规则（极其重要）
当问题涉及两个数值之间的"变化"、"差值"、"增减"时，必须严格按照以下规则确定减法顺序：
1. **"从A到B的变化" = B - A**（终点值减起点值）
   示例："从大型商业银行到外资银行的变化" = 外资银行值 - 大型商业银行值
2. **"A相比B的变化" = A - B**（前者减后者）
3. **"从第一季度到第四季度的变化" = 第四季度值 - 第一季度值**
4. 使用 calculate 工具做减法时，参数 a 是被减数，参数 b 是减数，结果 = a - b
   即：要计算 "B - A"，应调用 calculate(operation="subtract", a=B值, b=A值)
5. 计算结果为负数时，必须保留负号，不要取绝对值
6. 回答中必须先写出计算公式（如：外资银行 - 大型商业银行 = 121.77 - 12461.06 = -12339.29），再给出最终答案
